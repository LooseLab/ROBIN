...
!!! abstract "Read this before upgrading"
    **ROBIN 0.5** folds **Little John** into the main line, requires **Python 3.12** and a **fresh conda env** from **`robin.yml`**. Treat as a **full reinstall** — do not reuse older ROBIN or little_john environments. Review **upgrade requirements** below before you migrate.
...
- **MNP-Flex:** optional **Epignostix** upload for bedMethyl. Current
  deployments should use `MNPFLEX_USERNAME` / `MNPFLEX_PASSWORD`; the initial
  implementation used `MNPFLEX_USER` / `MNPFLEX_PASS`. See
