                            ...
                            with self.all_tab_panels:
                                for position in item.additions:
                                    with ui.tab_panel(position.name):
                                        ...
                                        ui.label(
                                            f"Position {position.name} is available"
                                        )
                                        ui.button(
                                            "Connect to this position",
                                            on_click=lambda p=position: self.connect_to_position(
                                                p
                                            ),
                                        )
        ...
        try:
            ...
            with target.card().classes("w-full"):
                    ...
                    with ui.row().classes("items-center"):
                        ...
                        with ui.column().classes("flex-grow"):
                            with ui.row().classes("items-baseline"):
                                ui.label(f"MinION - {self.position.name}").classes(
                            ...
                            self.sample_id_label = ui.label("Sample ID: -").classes(
                                "text-lg font-semibold text-blue-700 mt-2"
                            )
                            self.experiment_id_label = ui.label(
                                "Experiment ID: -"
                            ).classes("text-lg font-semibold text-blue-700")
                ...
                with ui.card().classes(
                    ...
                    ui.label("Device Status").classes(
                        "text-lg font-bold mb-3 text-gray-900"
                    )
                    with ui.grid().classes("grid-cols-3 gap-4"):
                        # Device Info
                        with ui.column().classes("space-y-2"):
                            ui.label("Device Information").classes(
                                "text-md font-semibold mb-2 text-gray-800"
                            )
                            ...
                            self.run_id_label = ui.label("Run ID: -").classes(
                                "text-sm text-gray-700"
                            )
                            self.run_state_label = ui.label("Run State: -").classes(
                                "text-sm text-gray-700"
                            )
                            self.run_start_time_label = ui.label(
                                "Start Time: -"
                            ).classes("text-sm text-gray-700")
                            self.acquisition_duration_label = ui.label(
                                "Run Duration: -"
                            ).classes("text-sm text-gray-700")
                            self.expected_duration_label = ui.label(
                                "Expected Duration: -"
                            ).classes("text-sm text-gray-700")
                            self.expected_finish_label = ui.label(
                                "Expected Finish: -"
                            ).classes("text-sm text-gray-700")
                            self.run_purpose_label = ui.label("Purpose: -").classes(
                                "text-sm text-gray-700"
                            )

                        # Flowcell Hardware
                        with ui.column().classes("space-y-2"):
                            ui.label("Flowcell Hardware").classes(
                                "text-md font-semibold mb-2 text-gray-800"
                            )
                            self.flowcell_label = ui.label("Flowcell: -").classes(
                                "text-sm text-gray-700"
                            )
                            self.flowcell_id_label = ui.label(
                                "Flow Cell ID: -"
                            ).classes("text-sm text-gray-700")
                            self.flowcell_type_label = ui.label("Type: -").classes(
                                "text-sm text-gray-700"
                            )
                            self.asic_label = ui.label("ASIC: -").classes(
                                "text-sm text-gray-700"
                            )
                            self.channel_label = ui.label("Channels: -").classes(
                                "text-sm text-gray-700"
                            )
                            self.user_specified_product_code_label = ui.label(
                                "User Product Code: -"
                            ).classes("text-sm text-gray-700")
                            self.user_specified_flow_cell_id_label = ui.label(
                                "User Flow Cell ID: -"
                            ).classes("text-sm text-gray-700")

                        # Flowcell Status
                        with ui.column().classes("space-y-2"):
                            ui.label("Flowcell Status").classes(
                                "text-md font-semibold mb-2 text-gray-800"
                            )
                            self.adapter_status = ui.label("Adapter: -").classes(
                                "text-sm text-gray-700"
                            )
                            self.temp_label = ui.label("Temperature: -").classes(
                                "text-sm text-gray-700"
                            )
                            self.usage_label = ui.label("Usage Count: -").classes(
                                "text-sm text-gray-700"
                            )
                            self.barcode_label = ui.label("Barcode Kit: -").classes(
                                "text-sm text-gray-700"
                            )
                            self.has_adapter_label = ui.label("Has Adapter: -").classes(
                                "text-sm text-gray-700"
                            )
                            self.adapter_id_label = ui.label("Adapter ID: -").classes(
                                "text-sm text-gray-700"
                            )
                            self.adapter_insertion_time_label = ui.label(
                                "Adapter Inserted: -"
                            ).classes("text-sm text-gray-700")

                # Yield Statistics
                with ui.card().classes("w-full p-4 mb-4 bg-white").bind_visibility_from(
                    self, "show"
                ):
                    ui.label("Yield Statistics").classes(
                        "text-lg font-bold mb-3 text-gray-900"
                    )
                    with ui.grid().classes("grid-cols-3 gap-6"):
                        # Read Statistics
                        with ui.card().classes("p-4 bg-gray-50 rounded-lg"):
                            ui.label("Read Statistics").classes(
                                "text-md font-semibold mb-2 text-gray-800"
                            )
                            with ui.column().classes("space-y-2"):
                                self.read_count_label = ui.label("Reads: 0").classes(
                                    "text-sm text-gray-700"
                                )
                                self.bases_label = ui.label("Bases: 0").classes(
                                    "text-sm text-gray-700"
                                )
                                self.n50_label = ui.label("N50: -").classes(
                                    "text-sm text-gray-700"
                                )
                                self.estimated_n50_label = ui.label(
                                    "Estimated N50: -"
                                ).classes("text-sm text-gray-700")

                        # Quality Metrics
                        with ui.card().classes("p-4 bg-gray-50 rounded-lg"):
                            ui.label("Quality Metrics").classes(
                                "text-md font-semibold mb-2 text-gray-800"
                            )
                            with ui.column().classes("space-y-2"):
                                self.pass_reads_label = ui.label(
                                    "Pass Reads: 0"
                                ).classes("text-sm text-gray-700")
                                self.fail_reads_label = ui.label(
                                    "Fail Reads: 0"
                                ).classes("text-sm text-gray-700")
                                self.mean_basecall_speed_label = ui.label(
                                    "Basecall Speed: -"
                                ).classes("text-sm text-gray-700")
                        ...
                        with ui.card().classes("p-4 bg-gray-50 rounded-lg"):
                            ...
                            with ui.column().classes("space-y-2"):
                                self.output_folder_label = ui.label(
                                    "Output: -"
                                ).classes("text-sm text-gray-700 break-all")
                                self.basecall_config_label = ui.label(
                                    "Basecall Config: -"
                                ).classes("text-sm text-gray-700 break-all")
                        ...
                        if progress:
                            ...
                            if hasattr(progress, "raw_per_channel"):
                                self.read_count_label.text = (
                                    f"Reads: {progress.raw_per_channel.acquired:,}"
                                )

                            if hasattr(progress, "basecalled_per_channel"):
                                basecalled = progress.basecalled_per_channel
                                if hasattr(basecalled, "pass_"):
                                    self.pass_reads_label.text = (
                                        f"Pass Reads: {basecalled.pass_:,}"
                                    )
                                if hasattr(basecalled, "fail"):
                                    self.fail_reads_label.text = (
                                        f"Fail Reads: {basecalled.fail:,}"
                                    )
                ...
                if info.state == manager_pb2.FlowCellPosition.State.STATE_RUNNING:
                    ...
                    if hasattr(info, "start_time"):
                        start_time = datetime.fromtimestamp(
                            info.start_time.seconds + info.start_time.nanos / 1e9
                        )
                        # Update start time label with formatted datetime
                        self.run_start_time_label.text = (
                        ...
                        self.acquisition_duration_label.text = (
                            f"Run Duration: {hours:.1f} hours"
                        )
                        ...
                        try:
                            ...
                            if current_acquisition:
                                logger.debug(
                                    f"Current acquisition: {current_acquisition}"
                                )
                                ...
                                if hasattr(current_acquisition, "config_summary"):
                                    ...
                                    logger.debug(
                                        f"Acquisition config summary: {config}"
                                    )

                                    if hasattr(config, "target_run_until"):
                                        target = config.target_run_until
                                        logger.debug(
                                            f"Target run until criteria: {target}"
                                        )
                                        ...
                                        if hasattr(target, "run_time"):
                                            ...
                                            self.expected_finish_label.text = (
                                                "Expected Finish: Not Available"
                                            )
                                    else:
                                        self.expected_duration_label.text = (
                                            "Expected Duration: Not Set (No target)"
                                        )
                                        self.expected_finish_label.text = (
                                            "Expected Finish: Not Available"
                                        )
                                    ...
                                    self.expected_duration_label.text = (
                                        "Expected Duration: Not Available (No config)"
                                    )
                                    self.expected_finish_label.text = (
                                        "Expected Finish: Not Available"
                                    )
                            ...
                            logger.error(
                                f"Error getting acquisition run info: {str(e)}"
                            )
                            logger.error(f"Error type: {type(e)}")
                            import traceback
                    ...
                    if hasattr(info, "config_summary"):
                        ...
                        if hasattr(config, "basecalling_enabled"):
                            config_info.append(
                                f"Basecalling: {'Enabled' if config.basecalling_enabled else 'Disabled'}"
                            )
                        ...
                        if hasattr(config, "basecalling_config_filename"):
                            config_info.append(
                                f"Config: {config.basecalling_config_filename}"
                            )

                        if config_info:
                            self.run_purpose_label.text = " | ".join(config_info)
                        else:
                            self.run_purpose_label.text = (
                                "No config information available"
                            )
                else:
                ...
                if hasattr(self, "yield_summary_info") and self.yield_summary_info:
                    ...
                    if hasattr(self.yield_summary_info, "__dict__"):
                        ...
                        for field in self.yield_summary_info.DESCRIPTOR.fields:
                            yield_info[field.name] = getattr(
                                self.yield_summary_info, field.name
                            )
                    else:
                        # Try to use it as is
                    ...
                    self.read_count_label.text = (
                        f"Reads: {safe_get(yield_info, 'read_count'):,}"
                    )
                    self.bases_label.text = (
                        f"Bases: {safe_get(yield_info, 'selected_raw_samples'):,}"
                    )
                ...
                if self.connection:
                    for info in self.connection.device.stream_flow_cell_info():
                        ...
                        if info.has_flow_cell:
                            ...
                            self.asic_label.text = (
                                f"ASIC: {info.asic_version} ({info.asic_id_str[:8]}...)"
                            )
                            self.channel_label.text = f"Channels: {info.channel_count:,} ({info.wells_per_channel} wells/ch)"

                            # Update user-specified information
                            user_product = (
                                info.user_specified_product_code
                                if info.user_specified_product_code
                                else "Not Set"
                            )
                            self.user_specified_product_code_label.text = (
                                f"User Product Code: {user_product}"
                            )

                            user_flow_cell = (
                                info.user_specified_flow_cell_id
                                if info.user_specified_flow_cell_id
                                else "Not Set"
                            )
                            self.user_specified_flow_cell_id_label.text = (
                                f"User Flow Cell ID: {user_flow_cell}"
                            )

                            # Update adapter information
                            self.has_adapter_label.text = (
                                f"Has Adapter: {'Yes' if info.has_adapter else 'No'}"
                            )
                            if info.has_adapter:
                                self.adapter_status.text = "Adapter: Connected"
                                self.adapter_status.classes("text-sm text-green-700")
                                self.adapter_id_label.text = (
                                    f"Adapter ID: {info.adapter_id}"
                                )
                                ...
                                self.adapter_insertion_time_label.text = (
                                    "Adapter Inserted: -"
                                )
                            ...
                            if info.temperature_offset:
                                ...
                                self.temp_label.text = (
                                    f"Temperature Offset: {temp_offset}°C"
                                )
                            else:
                                self.temp_label.text = (
                                    "Temperature Offset: Not Available"
                                )
                            ...
                            if info.barcode_kit:
                                self.barcode_label.text = (
                                    f"Barcode Kit: {info.barcode_kit}"
                                )
                                if info.barcodes:
            ...
            while not self.stopped():
                try:
                    ...
                    for info in self._stream_current_run:
                        ...
                        if (
                            info.state
                            == manager_pb2.FlowCellPosition.State.STATE_RUNNING
                        ):
                            self.Live_Run = True
                            if first_run:
                                logger.info(
                                    f"Run {self.Run_ID} seen on {self.position}"
                                )
                        else:
                            self.Live_Run = False
                            self.Run_ID = None
                            if first_run:
                                ...
                                logger.warning(
                                    f"Run {self.Run_ID} finished on {self.position}."
                                )
        ...
        try:
            while not self.stopped():
                try:
                        ...
                        if info.HasField("protocol_run_info"):
                            ...
                            if hasattr(info.protocol_run_info, "user_info"):
                                ...
                                if hasattr(user_info, "sample_id"):
                                    sample_id = (
                                        user_info.sample_id.value
                                        if hasattr(user_info.sample_id, "value")
                                        else "Not Set"
                                    )
                                    self.sample_id_label.text = (
                                ...
                                if hasattr(user_info, "protocol_group_id"):
                                    protocol_group_id = (
                                        user_info.protocol_group_id.value
                                        if hasattr(user_info.protocol_group_id, "value")
                                        else "Not Set"
                                    )
                                    self.experiment_id_label.text = (
                                        f"Experiment ID: {protocol_group_id}"
                                    )
                                else:
                                    self.experiment_id_label.text = (
                                        "Experiment ID: Not Set"
                                    )
                            ...
                            if hasattr(meta_info, "tags"):
                                if "kit" in meta_info.tags:
                                    self.running_kit = meta_info.tags[
                                        "kit"
                                    ].string_value
                                if "flow cell" in meta_info.tags:
                                    self.Flowcell_Type = meta_info.tags[
                                        "flow cell"
                                    ].string_value
                    ...
                    logger.error(
                        f"[Position.stream_instance_activity] Inner loop error: {str(e)}"
                    )
                    time.sleep(1)
