...
class APIClient:
    ...
    def authenticate(self, username: str, password: str, client_id: str, client_secret: str, grant_type: str = "", scope: str = "") -> None:
        """
    ...
    def upload_sample(self, file_path: str, sample_name: str, disclaimer_confirmed: bool) -> Any:
        """
    ...
    def get_sample_report(self, sample_id: int) -> Any:
        ...
        if response.status_code == 200:
            ...
            raise Exception(f"Failed to fetch the report: {response.status_code}, {response.text}")
    ...
    def delete_sample(self, sample_id: int) -> Any:
        ...
        """
        url = f"{self.base_url}/api/v1/samples/{sample_id}"
        response = requests.delete(url, headers=self.get_headers(), verify=self.verify_ssl)
        return response.json()
