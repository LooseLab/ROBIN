...
class PerformanceMetrics:
    ...
    * Time series visualization
    ...
    def __init__(self, output_dir: Union[str, Path], metrics: Optional[Dict[str, List]] = None):
        """
    ...
    def save_metrics(self) -> None:
        ...
        try:
            ...
            lengths = {key: len(values) for key, values in self.metrics.items() if isinstance(values, list)}
            ...
            perf_df = pd.DataFrame({
                ...
                'file_size_mb': [size / (1024 * 1024) if size is not None else None 
                ...
                with ui.button(icon="help_outline", color="gray").props("flat round"):
                    with ui.menu() as menu:
                        with ui.card().classes("p-4 text-sm"):
                            ui.label("Performance Metrics Legend").classes("font-medium mb-2")
                            ui.label("• BAM Merge Time: Time taken to merge BAM files and extract target regions").classes("mb-1")
                            ui.label("• DataFrame Merge Time: Time taken to merge and process coverage data").classes("mb-1")
                            ui.label("• Total Processing Time: Total time for complete BAM processing pipeline").classes("mb-1")
                            ...
                            ui.label("• Median: Middle value of all times").classes("mb-1")
    ...
    def _create_stats_card(self, title: str, metric_key: str, unit: str, description: str) -> None:
        ...
        with ui.card().classes("p-4 border rounded-lg shadow-sm") as card:
            ...
            with ui.column().classes("w-full") as stats_container:
                ...
                with ui.grid(columns=2).classes("w-full gap-1") as stats_grid:
                    ...
                    ui.label(f"Time ({unit})").classes("text-sm font-medium text-gray-600 text-right")
    ...
    def _create_time_series_chart(self) -> None:
        ...
        self.performance_chart = ui.echart({
            ...
            "legend": {
                "data": ["BAM Merge Time", "DataFrame Merge Time", "Total Processing Time"],
                "top": 35,
        ...
        try:
            ...
            for metric_key, card_data in self.stats_cards.items():
                if metric_key in self.metrics and self.metrics[metric_key]:
                    ...
                    values = np.array([v for v in self.metrics[metric_key] if v is not None])
                    if len(values) > 0:
            ...
            if 'timestamps' in self.metrics and self.metrics['timestamps']:
                ...
                valid_indices = [i for i, t in enumerate(self.metrics['timestamps']) if t is not None]
                ...
                if valid_indices:
                    ...
                    series_data = []
                    for series_name in ['bam_merge_times', 'df_merge_times', 'total_processing_times']:
