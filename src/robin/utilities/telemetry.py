...
class Telemetry:
    ...
    def collect_run_info(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"arguments": safe_args, "version": args.get("version", "unknown")}
    ...
    def send_telemetry(self, run_args: Dict[str, Any]) -> bool:
                ...
                "system_info": {
                    ...
                    "location": {
                        ...
                        "latlng": (
                            [
                                str(coord)
                                for coord in self._location_info.get("latlng", [])
                            ]
                            if self._location_info.get("latlng")
                            else None
                        ),
                    },
                    "version_info": self._version_info,
        ...
        telemetry_data = {"body": json.dumps(inner_data)}

        # Log the telemetry data
        logging.debug("\nTelemetry Data Being Collected:")
        logging.debug("--------------------------------")
        logging.info(
            "Telemetry data is collected anonymously to help us understand how ROBIN is used."
        )
        ...
        try:
            ...
            if response.status_code != 200:
                logging.warning(
                    f"Failed to send telemetry. Status code: {response.status_code}, Response: {response.text}"
                )
                return False
            ...
            if response_data.get("statusCode") == 400:
                logging.warning(
                    f"Failed to process telemetry. Response: {response.text}"
                )
                return False

            logging.info(
                f"Telemetry sent successfully. Status code: {response.status_code}"
            )
    ...
    def create_map_element(self) -> None:
        ...
        try:
            ...
            with ui.card().classes("w-192"):  # Double the width from w-96 to w-192
                ui.label("Your Location").classes("text-sky-600 dark:text-white").style(
                ...
                if (
                    ...
                    ui.label(f"ROBIN Instance at {location_desc}").classes(
    ...
    def send_run_telemetry(self, methnice_instance) -> bool:
        ...
        try:
            ...
            logging.debug(
                f"Time since start: {round((datetime.utcnow() - self.start_time).total_seconds() / 60, 1)} minutes"
            )
            ...
            reads_processed = (
                len(methnice_instance.robin.processed_files)
                if hasattr(methnice_instance.robin, "processed_files")
                else 0
            )
            sample_count = (
                len(methnice_instance.robin.samples)
                if hasattr(methnice_instance.robin, "samples")
                else 0
            )
            queue_size = (
                len(methnice_instance.robin.file_queue)
                if hasattr(methnice_instance.robin, "file_queue")
                else 0
            )
            ...
            logging.debug(
                f"Active Modules: {[m for m in ['sturgeon', 'forest', 'nanodx', 'pannanodx', 'cnv', 'fusion', 'coverage', 'mgmt'] if m not in methnice_instance.exclude]}"
            )
                ...
                "data": {
                    ...
                    "analysis_metrics": {
                        ...
                        "duration_seconds": round(
                            (datetime.utcnow() - self.start_time).total_seconds(), 2
                        ),
                        "active_modules": [
                            m
                            for m in [
                    ...
                    "configuration": {
                        ...
                        "adaptive_sampling": methnice_instance.readfish_toml
                        is not None,
                        "basecalling_config": (
                            {"model": methnice_instance.basecall_config}
                            if methnice_instance.basecall_config
                            else None
                        ),
                        "experiment_duration": methnice_instance.experiment_duration,
                    },
