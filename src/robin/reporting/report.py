                ...
                if not result.empty:
                    ...
                    for gene_group in gene_groups:
                        ...
                        if len(result_reads) > 1:
                            ...
                            num_cols = len(result_reads)  # Number of columns in the subplot grid
                            ...
                            for i, ax in enumerate(range(num_plots), start=1):
                                ...
                                if row == 1:
                                    ...
                                    for index, row in gene_table[
                                        gene_table["Seqid"].eq(chrom) & gene_table["Start"].le(end) & gene_table[
                                            "End"].ge(start)
                                    ].iterrows():
                                        if row["Type"] == "gene":
                                            features.append(
                                                GraphicFeature(
                                                    ...
                                                    color="#ffd700",
                                                    label=row["gene_name"],
                                                    fontdict={"family": "serif", "color": "black", "fontsize": 8},
                                                    #fontdict={"color": "black", "fontsize": 8},
                                                )
                                            )

                                    for index, row in (
                                            gene_table[gene_table["gene_name"].eq(data["gene"])
                                    ...
                                    features2 = []
                                    for index, row in df[df["chromosome"].eq(chrom)].sort_values(by="id").iterrows():
                                        features2.append(
                                            GraphicFeature(
                                    ...
                                    record2 = GraphicRecord(
                                        sequence_length=end - start, first_index=start, features=features2
                                    )
                                    ax = plt.gca()
                                    record2.plot(ax=ax)
                                    ax.xaxis.set_major_formatter(FuncFormatter(human_readable_format))
                                    ax.tick_params(axis="x", labelsize=8)  # Set font size for x-axis labels
                                    ax.set_xlabel(f'Position (Mb) - {chrom} - {data["gene"]}', fontsize=10)
                                    ax.set_title(f'{data["gene"]}', fontsize=10)  # Reduced font size for titles
                            ...
                            buf2 = io.BytesIO()
                            plt.savefig(buf2, format="jpg", dpi=300, bbox_inches="tight")
                            plt.close()
                            buf2.seek(0)
