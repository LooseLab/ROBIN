...
class PDFExtractor:
    ...
    def extract_from_pdf(self, pdf_path: str) -> Optional[Dict]:
        ...
        try:
            with open(pdf_path, "rb") as file:
                ...
                mgmt_status = re.search(r"Predicted MGMT promoter status= (\w+)", full_text)
                
                mgmt_info = {
                    "sites_used": int(mgmt_sites.group(1)) if mgmt_sites else None,
                    "average_methylation": float(mgmt_avg.group(1)) if mgmt_avg else None,
                ...
                if filename_match:
                    try:
                        timestamp = datetime.strptime(filename_match.group(1), "%Y%m%d_%H%M%S")
                    except ValueError:
                        pass
    ...
    def save_data(self, data: Dict) -> bool:
        ...
        try:
            ...
            if source_file and any(d.get('source_file') == source_file for d in existing_data):
                logger.info(f"Report {source_file} has already been processed, skipping")
                return True
    ...
    def process_existing_reports(self, reports_dir: str) -> int:
        ...
        try:
            ...
            if os.path.exists(self.data_file):
                with open(self.data_file, 'r') as f:
                    data = json.load(f)
                    processed_files = {d.get('source_file') for d in data if d.get('source_file')}
