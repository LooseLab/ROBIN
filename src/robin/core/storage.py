...
class StorageManager:
        ...
        if self.mainuuid in app.storage.general:
            if "samples" in app.storage.general[self.mainuuid]:
                if sample_id in app.storage.general[self.mainuuid]["samples"]:
                    app.storage.general[self.mainuuid]["samples"][sample_id]["status"] = status
    ...
    def add_file_to_sample(self, sample_id: str, file_path: str) -> None:
        ...
        if self.mainuuid in app.storage.general:
            if "samples" in app.storage.general[self.mainuuid]:
                if sample_id in app.storage.general[self.mainuuid]["samples"]:
                    if file_path not in app.storage.general[self.mainuuid]["samples"][sample_id]["files"]:
                        app.storage.general[self.mainuuid]["samples"][sample_id]["files"].append(file_path)
