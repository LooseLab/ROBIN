...
def _get_latest_bed_file(bed_dir: str, pattern: str) -> Optional[str]:
    ...
    for bed_file in bed_files:
        try:
            ...
            if latest_file is None or os.path.getmtime(bed_file) > os.path.getmtime(
                latest_file
            ):
                latest_file = bed_file
    ...
    try:
        ...
        if num_cols >= 6:
            ...
            col_names = ["chrom", "start", "end", "name"] + [
                f"col{i}" for i in range(4, num_cols)
            ]
            df = pd.read_csv(
                bed_path,
                sep="\t",
...
def _intersect_with_target_genes(
    regions_df: pd.DataFrame, target_bed_path: str
) -> pd.DataFrame:
    """
    ...
    if not os.path.exists(target_bed_path):
        logger.warning(
            f"Target BED file not found: {target_bed_path}, skipping intersection"
        )
        return regions_df
...
def _calculate_genome_coverage(
    bed_df: pd.DataFrame, fai_path: Optional[str] = None
    ...
    if not fai_path or not os.path.exists(fai_path):
        logger.warning(
            "FAI file not provided or not found - cannot calculate genome coverage"
        )
        return None
    ...
    if missing_chroms:
        logger.warning(
            f"Chromosomes in BED file not found in FAI file: {sorted(missing_chroms)}"
        )
    ...
    for strand in ["+", "-", "."]:
        ...
        if skipped_regions > 0:
            logger.debug(
                f"Skipped {skipped_regions} {strand} strand regions due to missing chromosomes in FAI file"
            )
    ...
    results = {
        ...
        "plus_strand_proportion": (
            strand_coverage["+"] / strand_total_genome["+"]
            if strand_total_genome["+"] > 0
            else 0.0
        ),
    ...
    results["total_proportion"] = (
        total_covered / total_possible_coverage if total_possible_coverage > 0 else 0.0
    )
    ...
    if reference:
        ...
        if os.path.exists(reference):
            try:
                ...
                logger.debug(
                    f"FAI file not found for {reference}, attempting to create it with samtools faidx"
                )
                ...
                if result.returncode == 0 and os.path.exists(fai_path):
                    ...
                    logger.warning(
                        f"Failed to create FAI index for {reference}: {result.stderr}"
                    )
...
def _generate_visualization_data(
    sample_dir: str, log_entries: List[Dict[str, Any]]
) -> None:
    """
...
def _master_bed_source_signature(
    ...
    for source_type, pattern in (
        ("cnv_regions", "new_file_*.bed"),
        ("cnv_breakpoints", "breakpoints_*.bed"),
        ("fusion_breakpoints", "fusion_breakpoints_*.bed"),
        ("master_bed_breakpoints", "master_bed_breakpoints_*.bed"),
    ):
        path = _get_latest_bed_file(bed_dir, pattern)
    ...
    if target_bed_path and os.path.exists(target_bed_path):
        sources.append(
            {"type": "target_panel", "sha256": _file_sha256(target_bed_path)}
        )

    return {"version": 1, "target_panel": target_panel, "sources": sources}


def _master_bed_state_path(master_bed_path: str) -> str:
    return f"{master_bed_path}.sources.json"


def _master_bed_is_current(
    master_bed_path: str,
    source_signature: Dict[str, Any],
) -> bool:
    if not os.path.exists(master_bed_path):
        return False
    try:
        with open(_master_bed_state_path(master_bed_path), "r") as state_file:
            return json.load(state_file) == source_signature
    except (OSError, ValueError, TypeError):
        return False


def _save_master_bed_state(
    master_bed_path: str,
    source_signature: Dict[str, Any],
) -> None:
    temporary_path = f"{_master_bed_state_path(master_bed_path)}.tmp"
    with open(temporary_path, "w") as state_file:
        json.dump(source_signature, state_file, indent=2, sort_keys=True)
    os.replace(temporary_path, _master_bed_state_path(master_bed_path))


def _build_master_bed_data(
    sample_id: str,
    work_dir: str,
    analysis_counter: int,
    target_panel: Optional[str],
    ...
    log: logging.Logger,
) -> Optional[Tuple[pd.DataFrame, Dict[str, Dict[str, float]]]]:
    """
    Build master BED DataFrame and coverage data from all BED sources (no lock).
    Caller must hold lock only when writing the result to disk.
    """
    sample_dir = os.path.join(work_dir, sample_id)
    ...
    if cnv_bed:
        ...
        if not cnv_df.empty:
            ...
            log.debug(
                f"Loaded {len(cnv_df)} CNV regions (converted to {len(cnv_expanded_df)} breakpoint regions)"
            )
            ...
            if fai_path:
                ...
                if cnv_coverage:
                    cnv_entry.update(
                        {
                            "total_proportion": cnv_coverage.get(
                                "total_proportion", 0.0
                            ),
                            "plus_strand_proportion": cnv_coverage.get(
                                "plus_strand_proportion", 0.0
                            ),
                            "minus_strand_proportion": cnv_coverage.get(
                                "minus_strand_proportion", 0.0
                            ),
                        }
                    )
    ...
    if breakpoints_bed:
        ...
        if not bp_df.empty:
            ...
            log.debug(
                f"Loaded {len(bp_df_processed)} CNV breakpoints (with both strands)"
            )
            ...
            if fai_path:
                ...
                if bp_coverage:
                    bp_entry.update(
                        {
                            "total_proportion": bp_coverage.get(
                                "total_proportion", 0.0
                            ),
                            "plus_strand_proportion": bp_coverage.get(
                                "plus_strand_proportion", 0.0
                            ),
                            "minus_strand_proportion": bp_coverage.get(
                                "minus_strand_proportion", 0.0
                            ),
                        }
                    )
    ...
    if fusion_bed:
        ...
        if not fusion_df.empty:
            ...
            log.debug(
                f"Loaded {len(fusion_df_processed)} fusion breakpoints (preserving strand)"
            )
            ...
            if fai_path:
                ...
                if fusion_coverage:
                    fusion_entry.update(
                        {
                            "total_proportion": fusion_coverage.get(
                                "total_proportion", 0.0
                            ),
                            "plus_strand_proportion": fusion_coverage.get(
                                "plus_strand_proportion", 0.0
                            ),
                            "minus_strand_proportion": fusion_coverage.get(
                                "minus_strand_proportion", 0.0
                            ),
                        }
                    )
    ...
    if master_bed_bp:
        ...
        if not master_bed_bp_df.empty:
            ...
            log.debug(
                f"Loaded {len(master_bed_bp_df_processed)} master BED breakpoints (with both strands)"
            )
            master_bed_bp_df_merged = _merge_overlapping_regions(
                master_bed_bp_df_processed
            )
            master_bed_bp_entry = {"region_count": len(master_bed_bp_df_processed)}
            if fai_path:
                master_bed_bp_coverage = _calculate_genome_coverage(
                    master_bed_bp_df_merged, fai_path
                )
                if master_bed_bp_coverage:
                    master_bed_bp_entry.update(
                        {
                            "total_proportion": master_bed_bp_coverage.get(
                                "total_proportion", 0.0
                            ),
                            "plus_strand_proportion": master_bed_bp_coverage.get(
                                "plus_strand_proportion", 0.0
                            ),
                            "minus_strand_proportion": master_bed_bp_coverage.get(
                                "minus_strand_proportion", 0.0
                            ),
                        }
                    )
    ...
    if target_panel:
        ...
        if target_bed_path:
            log.debug(
                f"Loading target panel regions: {target_panel} ({target_bed_path})"
            )
            target_df = _load_bed_file(target_bed_path, require_bed6=True)
            if not target_df.empty:
                ...
                if fai_path:
                    target_coverage = _calculate_genome_coverage(
                        target_df_merged, fai_path
                    )
                    if target_coverage:
                        target_entry.update(
                            {
                                "total_proportion": target_coverage.get(
                                    "total_proportion", 0.0
                                ),
                                "plus_strand_proportion": target_coverage.get(
                                    "plus_strand_proportion", 0.0
                                ),
                                "minus_strand_proportion": target_coverage.get(
                                    "minus_strand_proportion", 0.0
                                ),
                            }
                        )
                ...
                log.debug(
                    f"Keeping all {len(merged_df)} breakpoint regions and adding {len(target_df_processed)} target gene regions"
                )
                all_regions_with_target = pd.concat(
                    [merged_df, target_df_processed], ignore_index=True
                )
                merged_df = _merge_overlapping_regions(all_regions_with_target)
                log.debug(
                    f"Final merged regions including target genes: {len(merged_df)}"
                )
            else:
        ...
        log.debug(
            "No target panel specified - master BED will include all breakpoint regions"
        )
            ...
            master_entry.update(
                {
                    ...
                    "plus_strand_proportion": master_coverage.get(
                        "plus_strand_proportion", 0.0
                    ),
                    "minus_strand_proportion": master_coverage.get(
                        "minus_strand_proportion", 0.0
                    ),
                }
            )
    coverage_data["master_bed"] = master_entry
    return (sorted_df, coverage_data)
    ...
    try:
        for attempt in range(2):
            ...
            with FileLock(lock_file, timeout=lock_timeout):
                ...
                if latest_master and _master_bed_is_current(
                    latest_master, source_signature
                ):
            ...
            if not build_result:
                return None
            sorted_df, coverage_data = build_result
            bed6_cols = ["chrom", "start", "end", "name", "score", "strand"]
            ...
            with FileLock(lock_file, timeout=lock_timeout):
                ...
                if current_signature != source_signature:
                    log.debug(
                        f"Master BED sources changed during generation for {sample_id}"
                    )
                    continue
                latest_master = _get_latest_bed_file(bed_dir, "master_*.bed")
                if latest_master and _master_bed_is_current(
                    latest_master, current_signature
                ):
                    log.debug(
                        f"Master BED was generated by another process: {latest_master}"
                    )
                    return latest_master
                ...
                master_bed_path = os.path.join(
                    bed_dir, f"master_{write_counter:03d}.bed"
                )
                ...
                os.replace(temporary_path, master_bed_path)
                _save_master_bed_state(master_bed_path, current_signature)
                if coverage_data:
        ...
        log.warning(
            ...
            "leaving regeneration for the next update",
            sample_id,
        )
        return None
    ...
    try:
        ...
        if cnv_regions_file and os.path.exists(cnv_regions_file):
            ...
            if not cnv_df.empty:
                ...
                log.debug(
                    f"Loaded {len(cnv_df)} CNV regions (converted to {len(expanded_df)} breakpoint regions)"
                )
        ...
        if fusion_breakpoints_file and os.path.exists(fusion_breakpoints_file):
            ...
            if not fusion_df.empty:
                ...
                log.debug(
                    f"Loaded {len(fusion_df)} fusion breakpoints (preserving strand)"
                )
        ...
        if master_bed_breakpoints_file and os.path.exists(master_bed_breakpoints_file):
            log.debug(
                f"Loading master BED breakpoints from: {master_bed_breakpoints_file}"
            )
            ...
            if not master_bed_bp_df.empty:
                ...
                log.debug(
                    f"Loaded {len(master_bed_bp_df)} master BED breakpoints (with both strands)"
                )
        ...
        if target_bed_file and os.path.exists(target_bed_file):
            ...
            if not target_df.empty:
                ...
                log.debug(
                    f"Keeping all {len(merged_df)} breakpoint regions and adding {len(target_df)} target gene regions"
                )

                # Combine all breakpoints with target gene regions
                all_regions_with_target = pd.concat(
                    [merged_df, target_df], ignore_index=True
                )

                log.debug(
                    f"Combined {len(merged_df)} breakpoints with {len(target_df)} target gene regions"
                )
        ...
        log.info(
            f"Generated master BED file: {output_file} with {len(sorted_df)} regions"
        )
        return output_file
...
Examples:
  ...
  python master_bed_generator.py --cnv-regions new_file_001.bed \\
    ...
    parser.add_argument(
        "--output", type=str, required=True, help="Path to output master BED file"
    )
    parser.add_argument(
        "--expand-size",
        type=int,
    ...
    if not any(
        [
            args.cnv_regions,
            args.cnv_breakpoints,
            args.fusion_breakpoints,
            args.master_bed_breakpoints,
        ]
    ):
        parser.error(
            ...
            try:
                ...
                if not bed_df.empty:
                    ...
                    if coverage_stats:
                        ...
                        print("=" * 60)
                        print(
                            f"Total genome size: {coverage_stats['total_genome_size']:,} bp"
                        )
                        print(f"\nCoverage by strand:")
                        print(
                            f"  + strand: {coverage_stats['plus_strand_coverage']:,} bp "
                            f"({coverage_stats['plus_strand_proportion']*100:.4f}%)"
                        )
                        print(
                            f"  - strand: {coverage_stats['minus_strand_coverage']:,} bp "
                            f"({coverage_stats['minus_strand_proportion']*100:.4f}%)"
                        )
                        if coverage_stats["unstranded_coverage"] > 0:
                            print(
                                f"  . strand: {coverage_stats['unstranded_coverage']:,} bp "
                        ...
                        print(
                            f"Total proportion covered: {coverage_stats['total_proportion']*100:.4f}%"
                        )
                        print("=" * 60)
                    else:
                        print(
                            "Warning: Could not calculate genome coverage statistics",
                            file=sys.stderr,
                        )
                else:
                    print(
                        "Warning: Generated BED file is empty, cannot calculate coverage",
                        file=sys.stderr,
                    )
            except Exception as e:
                logger.error(f"Error calculating genome coverage: {e}")
                print(
                    f"Warning: Error calculating genome coverage: {e}", file=sys.stderr
                )
