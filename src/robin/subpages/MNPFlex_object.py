...
class MNPFlex_Object(BaseAnalysis):
    ...
    **kwargs
    ...
    def __init__(self, *args, mnpflex_config: Optional[Dict[str, Any]] = None, **kwargs):
    ...
    def initialize_mnpflex_client(self) -> None:
        ...
        try:
            self.mnpflex_client = MnpFlexClient(base_url="https://mnp-flex.org", verify_ssl=True)
    ...
    def create_dynamic_classification_chart(self, key: str, y_max: float = 1) -> ui.echart:
    ...
    def update_summary_card(self):
        ...
        if not (self._summary_status_label and self._summary_reports_label and self._summary_last_label):
            return
        ...
        try:
            ...
            if mnpflex_reports:
                ...
                self._summary_reports_label.text = f"Reports Generated: {len(mnpflex_reports)}"
                # Find latest report
                latest_report = max(mnpflex_reports, key=lambda f: os.path.getmtime(os.path.join(sample_dir, f)))
                latest_time = os.path.getmtime(os.path.join(sample_dir, latest_report))
                dt = datetime.fromtimestamp(latest_time)
                self._summary_last_label.text = f"Last Analysis: {dt.strftime('%Y-%m-%d %H:%M:%S')}"
            else:
            ...
            if os.path.exists(json_path):
                ...
                if data:
                    ...
                    for key in ["superfamily", "family", "class", "subclass"]:
                        label = "--"
                        score = ""
                        if "classification" in latest_entry and key in latest_entry["classification"]:
                            label = latest_entry["classification"][key].get("label", "--")
                            score_val = latest_entry["classification"][key].get("score")
                            if score_val is not None:
                                score = f" ({score_val:.2f})"
                        if key in self._summary_classification_labels:
                            self._summary_classification_labels[key].text = f"{key.title()}: {label}{score}"
                    # Update MGMT label
                    mgmt = latest_entry.get("mgmt_info", {})
                    mgmt_status = mgmt.get("status", "--")
                    avg = mgmt.get("average_methylation")
                    if avg is not None:
                        self._summary_mgmt_label.text = f"MGMT: {mgmt_status} (Avg: {avg:.1f}%)"
                    else:
            ...
            self._summary_status_label.text = "Status: Error"
    ...
    def update_extracted_data_charts(self) -> None:
        ...
        try:
            ...
            logger.error(f"Error updating extracted data charts: {str(e)}", exc_info=True)
    ...
    async def process_bam(self, bamfile: str, timestamp: float) -> None:
        ...
        try:
            ...
            while result_status == "initialized":
                await ui.run_javascript('await new Promise(resolve => setTimeout(resolve, 1000))')
                sample = self.mnpflex_client.get_sample(sample_id)
                result_status = sample["bed_file_sample"]["analysis_status"]
                ...
                with ui.card().classes("w-full p-4 mb-4"):
                    with ui.row().classes("w-full items-center justify-between"):
                        ...
                        with ui.column().classes("gap-2"):
                            ui.label("MNP-FLEX Methylation Analysis").classes("text-lg font-medium")
                            # RESEARCH USE ONLY indicator
                            with ui.row().classes("items-center gap-2"):
                                ui.icon("warning").classes("text-red-600 text-base")
                                ui.label("RESEARCH USE ONLY").classes("text-red-700 font-semibold text-sm")
                            with ui.row().classes("items-center gap-2"):
                                self._summary_status_label = ui.label("Status: Awaiting Data").classes("text-gray-600")
                                ui.label("--").classes("px-2 py-1 rounded bg-gray-100 text-gray-600")
                            # Latest classification results
                            for key in ["superfamily", "family", "class", "subclass"]:
                                self._summary_classification_labels[key] = ui.label(f"{key.title()}: --").classes("text-gray-700 text-sm")
                            self._summary_mgmt_label = ui.label("MGMT: --").classes("text-gray-700 text-sm")

                        # Right side - Analysis metrics
                        with ui.column().classes("gap-2 text-right"):
                            ui.label("Analysis Details").classes("font-medium")
                            self._summary_reports_label = ui.label("Reports Generated: --").classes("text-gray-600")
                            self._summary_last_label = ui.label("Last Analysis: --").classes("text-gray-600")

                    # Bottom row - Information
                    with ui.row().classes("w-full mt-4 text-sm text-gray-500 justify-center"):
                        ui.label("Methylation-based classification using MNP-FLEX platform")
        ...
        with ui.card().classes("w-full mb-4 bg-red-50 border-2 border-red-200"):
            with ui.row().classes("items-center gap-2"):
                ...
                with ui.column().classes("gap-1"):
                    ui.label("⚠️ IMPORTANT WARNING").classes("text-red-800 font-bold text-lg")
                    ui.label("MNP-FLEX is for RESEARCH USE ONLY").classes("text-red-700 font-semibold")
                    ui.label("Users must have their own account with Epignostix to use this service").classes("text-red-700")
                    ui.link("https://mnp-flex.org/", "Visit MNP-FLEX website", new_tab=True).classes("text-red-600 hover:text-red-800")
                ...
                if mnpflex_reports:
                    ...
                    for report in mnpflex_reports:
                        try:
                            ...
                            logger.error(f"Error parsing report filename {report}: {str(e)}")
                            continue
