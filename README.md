...
Options:
  ...
  --centreid TEXT                 Provide an identifier to be used in the
                                  experiment name field.  [required]
  ...
  -r, --reference FILE            Path to the reference genome and index.
                                  [required]
  -b, --bed_file FILE             Path to the bedfile to use for adaptive
                                  sampling.  [required]
  --basecall_config TEXT          The basecalling configuration to use.
                                  [required]
  -rt, --readfish_toml FILE       Path to the TOML file used to control
                                  readfish.
  --experiment_duration INTEGER   The experiment run time in hours.
                                  [required]
  --browse                        Browse Historic Data.
  -e, --exclude [sturgeon|forest|nanodx|pannanodx|cnv|fusion|coverage|mgmt]
                                  Exclude analysis types with one or more of
                                  these options.
  -w, --watchfolder DIRECTORY
  --no-telemetry                  Opt out of sending anonymous usage
                                  statistics.
  --enable-snp-calling            Enable SNP calling functionality (requires
                                  reference genome).
...
You must use hg38 - we recommend using a non-alt version for alignment. You should use the same reference as you align your files to in minKNOW.

ROBIN expects Aligned, Methylation High Accuracy Called BAM files as output by minKNOW during a sequencing run. We recommend writing one BAM file every 10,000 reads for real time calling.
...
```console
robin --threads 1  -r ~/references/hg38_simple.fa /path/to/watchfolder /path/to/output
```
...
![img.png](images/home_page_0.3.png)
...
![img.png](images/LivePage_Overview_0.3.png)
...
![img.png](images/Top_Level_0.3.png)
